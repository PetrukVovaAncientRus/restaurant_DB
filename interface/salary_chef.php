...
$title = "Інформація про зарплатню кухарів";
...
// Виведення таблиці
$query = "SELECT admin_id, chef_id, amount FROM salary_chef";
...
echo "<center>";
echo "<table border=\"1\">";
echo "<tr><th>admin id</th><th>chef id</th><th>amount</th></tr>";
...
if (isset($_POST['insert'])) {
    ...
    $valid = validateInput($data[1], "admin_id") && validateInput($data[2], "chef_id") && validateInput($data[3], "amount") &&
             validateInteger($data[1], "admin_id") && validateInteger($data[2], "chef_id") && validateInteger($data[3], "amount");

    if ($valid) {
        $insert_Query = "INSERT INTO salary_chef (admin_id, chef_id, amount) VALUES (?, ?, ?)";
...
if (isset($_POST['delete'])) {
    ...
    if ($valid) {
        $delete_Query = "DELETE FROM salary_chef WHERE id = ?";
...
if (isset($_POST['update'])) {
    $data = getPosts();
    $valid = validateInput($data[0], "id") && validateInput($data[1], "admin_id") && validateInput($data[2], "chef_id") &&
             validateInput($data[3], "amount") && validateInteger($data[0], "id") &&
             validateInteger($data[1], "admin_id") && validateInteger($data[2], "chef_id") && validateInteger($data[3], "amount");

    if ($valid) {
        $update_Query = "UPDATE salary_chef SET admin_id = ?, chef_id = ?, amount = ? WHERE id = ?";
...
<form action="salary_chef.php" method="post"><br><br>
