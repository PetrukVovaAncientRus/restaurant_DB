...
function getPosts() {
    ...
    $posts[1] = isset($_POST['ordered_dishes']) ? sanitizeInput($_POST['ordered_dishes']) : "";
    $posts[2] = isset($_POST['client_id']) ? sanitizeInput($_POST['client_id']) : "";
    return $posts;
}

// Виведення таблиці
$query = "SELECT ordered_dishes, client_id FROM order_";
...
echo "<center>";
echo "<table border=\"1\">";
echo "<tr><th>ordered dishes</th><th>client id</th></tr>";
while ($row = $result->fetch_assoc()) {
    echo "<tr>";
    echo "<td>" . $row["ordered_dishes"] . "</td>";
...
if (isset($_POST['insert'])) {
    ...
    $valid = validateInput($data[1], "ordered_dishes") && validateInput($data[2], "client_id") &&
             validateInteger($data[1], "ordered_dishes") && validateInteger($data[2], "client_id");

    if ($valid) {
        $insert_Query = "INSERT INTO order_ (ordered_dishes, client_id) VALUES (?, ?)";
...
if (isset($_POST['delete'])) {
    ...
    if ($valid) {
        $delete_Query = "DELETE FROM order_ WHERE id = ?";
...
if (isset($_POST['update'])) {
    $data = getPosts();
    $valid = validateInput($data[0], "id") && validateInput($data[1], "ordered_dishes") &&
             validateInput($data[2], "client_id") && validateInteger($data[0], "id") &&
             validateInteger($data[1], "ordered_dishes") && validateInteger($data[2], "client_id");

    if ($valid) {
        $update_Query = "UPDATE order_ SET ordered_dishes = ?, client_id = ? WHERE id = ?";
...
<form action="order_.php" method="post"><br><br>
    <input type="number" name="id" placeholder="id" value="<?php echo $id; ?>"><br><br>
    <input type="number" name="ordered_dishes" placeholder="ordered_dishes" value="<?php echo $ordered_dishes; ?>"><br><br>
